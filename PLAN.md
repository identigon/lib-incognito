...
- [ ] Minor: the `DpiaArtifactEmitter` is an opt-in utility (not auto-invoked by the pipeline) — by design, but worth a doc note.
...
- Testcontainers PostgreSQL benchmark suites:
  ...
  - [ ] **Pagila (Sakila)** — *optional; deprioritised.* Its distinctive coverage (composite `film_actor`, VIEW exclusion, passthrough audit) is already met by `CompositeKeyE2ETest` / `ViewExclusionE2ETest` / `PassthroughAuditE2ETest` + the wired benchmarks. Two blockers found in the vendored copy: it was fetched from `master` (unpinned → version drift; the "33-table" releases have since grown to 71 tables, 55 of them `payment_p*` partitions), and it now needs the **pgvector** extension for `film_embedding` (not in stock `postgres:16-alpine`). If wired, pin an **older, pre-pgvector Pagila tag** (fewer partitions, no extension) rather than `master`. **DVD Rental** was evaluated and rejected: an independent Sakila port (not a Pagila subset) but its origin (postgresqltutorial.com) publishes no clear licence — a provenance dead-end. If a distinct commerce-domain benchmark is wanted, prefer **Chinook** (MIT, JDBC-loadable plain SQL) over another Sakila port.
