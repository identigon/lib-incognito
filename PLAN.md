...
- Testcontainers PostgreSQL benchmark suites:
  ...
  - [x] **Pagila (Sakila)** — `PagilaBenchmarkE2ETest`, pinned to tag **`pagila-v3.0.0`**
    (pre-pgvector: 22 tables, no extension) rather than `master` (needs the **pgvector** extension
    for `film_embedding` + has 55 `payment_p*` partitions). Clones the 15 non-partitioned Sakila
    core tables; the partitioned `payment` is excluded from the policy. Schema **and** the ~5 MB
    `pagila-insert-data.sql` are both vendored (`benchmarks/SOURCES.md`) — previously only the
    schema was vendored and the data file was fetched over the network at test time with a SHA-256
    check, skipping (not failing) the whole benchmark with no network. Reconsidered: GitHub only
    hard-blocks files over 100 MB (warns above 50 MB), so 5 MB was never actually a repo-size
    problem, and the network fetch meant this one benchmark's substantial coverage could vanish in
    any network-restricted CI environment with no failure signal. The SHA-256 is kept in
    `SOURCES.md` as a provenance record of the exact upstream bytes at vendor time, not re-checked
    at test time — git/GitHub already guarantee the committed bytes, the same as every other
    vendored fixture, none of which get a runtime hash check either. `OWNER TO postgres` is stripped
    at load; source data loads under `session_replication_role='replica'` to bypass the dump's
    non-FK-ordered rows. Exercises two composite PKs (`film_actor`, `film_category`), seven excluded
    views, opaque `bytea`/`tsvector`/array passthrough, and `email` fabrication. **Surfaced and
    fixed** a real engine gap: a kept **enum** / user-type column failed on re-insert (a read
    `String` bound as `varchar` → type mismatch), so no table with an enum could be cloned — now
    `String` values bind as `Types.OTHER` and PostgreSQL casts to the column's type. **DVD Rental**
    (postgresqltutorial) was re-evaluated and rejected again: no citable licence + binary
    `pg_restore` format (not our JDBC statement-stream harness).
...
- [x] **Benchmark-fixture provenance/licence done** — re-sourced from canonical upstreams with
  recorded download + licence URLs; verbatim licence texts under `benchmarks/LICENCES/`, attribution
  in `benchmarks/NOTICE`, provenance in `benchmarks/SOURCES.md`. Licences: PetClinic Apache-2.0,
  Pagila PostgreSQL License, Northwind Ms-PL, Employees CC BY-SA 3.0 (share-alike honoured: same
  licence, full attribution chain, changes noted in the file header), Chinook MIT. All five
  fixtures are vendored (see the Pagila bullet above for the one that changed).
- [x] **Benchmark harness deduplicated + DPIA artifact emission demonstrated** — extracted the
  ~150–200 lines of near-identical boilerplate that had accumulated across all 5 benchmark tests
  (Docker-availability check, classpath-resource/policy loading, a JDBC `DataSource` stub,
  `scalar()`, target-database creation) into `benchmark.BenchmarkSupport`, a package-private
  static-method utility with well-named methods (`assumeDockerAvailable`, `resource`, `loadPolicy`,
  `dataSource`, `execute`, `createTargetDatabase`, `scalar`, `emitAndVerifyDpiaReport`). Each
  benchmark's own file now reads as just its schema/fixture quirk and its assertions. Each benchmark
