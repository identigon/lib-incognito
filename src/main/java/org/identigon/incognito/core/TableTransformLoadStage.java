...
public final class TableTransformLoadStage implements PipelineStage {
    ...
    public StageResult process(PipelineContext context) throws IncognitoException {
        ...
        List<org.identigon.incognito.engine.DialectHandler.DroppedForeignKey> droppedForeignKeys = List.of();
        if (!plan.cyclicTablesToUpdatePass2().isEmpty()) {
            try (Connection probe = context.target().getConnection()) {
                org.identigon.incognito.engine.DialectHandler dialect = getDialectHandler(probe);
    ...
    private long processTable(
        ...
        try (Connection sourceConn = context.source().getConnection();
            ...
            org.identigon.incognito.engine.DialectHandler dialect = getDialectHandler(targetConn);
                ...
                try (ResultSet rs = stmt.executeQuery(selectSql);
                    ...
                    while (rs.next()) {
                            ...
                            if (compositePk && sourcePk != null && allNonNull(newPkVals)) {
                                Object newKey = new org.identigon.incognito.api.CompositeKey(newPkVals);
    ...
    private ColumnTransformer buildFkTransformer(
        ...
        return (value, rs, pk, sqlType, counter, ctx, meta) -> {
            ...
            if (mapped.isPresent() && mapped.get() instanceof org.identigon.incognito.api.CompositeKey ck) {
    ...
    private ColumnTransformer buildDirectIdTransformer(
        ...
        Transformation<String> transformation = switch (strategy) {
            ...
            case ALTEREGO_FIRST_NAME -> alterEgo.firstName();
            case ALTEREGO_LAST_NAME -> alterEgo.lastName();
    ...
    /**
     * A row's source primary key: the single column's value, a {@link org.identigon.incognito.api.CompositeKey}
    ...
    private static Object buildSourceKey(ResultSet rs, List<String> pkCols, List<String> columnsToProcess) throws SQLException {
        ...
        return new org.identigon.incognito.api.CompositeKey(vals);
    }
    ...
    private org.identigon.incognito.engine.DialectHandler getDialectHandler(Connection conn) throws SQLException {
